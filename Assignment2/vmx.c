...
//Import atomic exit_processing_time -Preethi T G
extern atomic64_t exit_processing_time;
//End of import - Preethi T G
...
static int vmx_handle_exit(struct kvm_vcpu *vcpu)
{
	...
	int result = 0;
	//Declare and initialize variables to store rdtsc before and after exit emulation - Preethi T G
	u64 start_exit_time = 0;
	u64 end_exit_time = 0;
	u64 delta = 0;
	//End of declaration - Preethi T G

	trace_kvm_exit(exit_reason, vcpu, KVM_ISA_VMX);

	atomic_inc(&exit_count);
	printk(KERN_WARNING "\nhello exit count for this vpid: %u", vcpu->vcpu_id);
	printk(KERN_WARNING "\n hello thy set new value for exit counter: %u\n", atomic_read(&exit_count));
	...
	if (exit_reason < kvm_vmx_max_exit_handlers
		...
		//Code to calculate total time spent processing all exits - Preethi T G
		start_exit_time = rdtsc();
		printk(KERN_WARNING "\nPreethi start_exit_time: %llu\n",start_exit_time);
		result = kvm_vmx_exit_handlers[exit_reason](vcpu);
		end_exit_time = rdtsc();
		printk(KERN_WARNING "\nPreethi end_exit_time: %llu\n",end_exit_time);
		delta = end_exit_time - start_exit_time; 
		printk(KERN_WARNING "\nPreethi delta: %llu\n",delta);
		atomic64_add(delta, &exit_processing_time);
		printk(KERN_WARNING "\nhello total exit time for this vpid: %u", vcpu->vcpu_id);
		printk(KERN_WARNING "\nPreethi Total time spent: %llu\n",atomic64_read(&exit_processing_time));
		return result;
		//End of change by Preethi T G
		//TODO: Preethi: add code to rdtsc here
		// TODO: Preethi: after calculate cycles for each exit, add it to the extern variable to calculate the sum
		// TODO: Preethi: check other if case to see if we need to handle any other cases
		/*Begin of comment - Preethi T G
		return kvm_vmx_exit_handlers[exit_reason](vcpu);
		End of comment - Preethi T G */
	}
