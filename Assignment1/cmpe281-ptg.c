 ...
 * Hoang Thy Vo - Preethi Thimma Govarthanarajan
 */
...
/*
TODO Preethi: add required MSRs
*/
...
/*
 * Primary processor capabilities
 * See SDM volume 3, section 24.6.2, Table 24-6.
 */
struct capability_info processor[21] =
{
...
/*
 * Secondary processor capabilities
 * See SDM volume 3, section 24.6.2, Table 24-7.
 */
struct capability_info secondary_processor[23] =
{
 ...
 * See SDM volume 3, section 24.8, Table 24-12.
 */
...
/**
TODO: double check table 24-13 and table 24-14 - Done
**/
};

/*
 * VM-Exit capabilities
 * See SDM volume 3, section 24.7, Table 24-10.
 */
...
/**
TODO: double check table 24-11 - Done
**/
};
...
detect_vmx_features(void)
{
	...
	pr_info("IA32_VMX_BASIC MSR: 0x%llx\n",
		 	(uint64_t)(lo | (uint64_t)hi << 32));

	/*
	What is true controls?
	write code to determine if true controls are available -> call the correct MSR
	...
	*/
 	...
 	*
 	* Read the IA32_VMX_BASIC MSR, check if bit 55 is set
 	*/
	rdmsr(IA32_VMX_BASIC, lo, hi);
	if (hi & (1UL << (55-32))) {
	 	pr_info("BIT 55 of IA32_VMX_BASIC MSR SET\n");
		/* Pinbased true controls */
		rdmsr(IA32_VMX_TRUE_PINBASED_CTLS, lo, hi);
		pr_info("Pinbased True Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
		report_capability(pinbased, 5, lo, hi);
		

		/* Processor true controls */
		rdmsr(IA32_VMX_TRUE_PROCBASED_CTLS, lo, hi);
		pr_info("Processor True Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
		report_capability(processor, 21, lo, hi);
		


		/* VM-Entry true controls */
		rdmsr(IA32_TRUE_ENTRY_CTLS, lo, hi);
		pr_info("VM-Entry True Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
		report_capability(vmentry, 9, lo, hi);


		/* VM-Exit true controls */
		rdmsr(IA32_VMX_TRUE_EXIT_CTLS, lo, hi);
		pr_info("VM-Exit True Controls MSR: 0x%llx\n",
			(uint64_t)(lo | (uint64_t)hi << 32));
		report_capability(vmexit, 11, lo, hi);

	} else {
		pr_info("BIT 55 of IA32_VMX_BASIC MSR NOT SET\n");

		/* Pinbased controls */

        rdmsr(IA32_VMX_PINBASED_CTLS, lo, hi);
        pr_info("IA32_VMX_PINBASED_CTLS MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(pinbased, 5, lo, hi);
        ...
        pr_info("IA32_VMX_PROCBASED_CTLS MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(processor, 21, lo, hi);
        ...
        if(hi & (1<<31)){
            pr_info("BIT 63 OF IA32_VMX_PROCBASED_CTLS IS SET. SECONDARY PROCESSOR BASED CONTROLS ARE AVAILABLE\n");
            rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
            pr_info("IA32_VMX_PROCBASED_CTLS2 MSR: 0x%llx\n",
                    (uint64_t)(lo | (uint64_t)hi << 32));
            report_capability(secondary_processor, 23, lo, hi);
        }
        else{
            pr_info("BIT 63 OF IA32_VMX_PROCBASED_CTLS IS NOT SET. SECONDARY PROCESSOR BASED CONTROLS ARE UNAVAILABLE\n");
        }
        ...
        pr_info("IA32_VMX_ENTRY_CTLS MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vmentry, 9, lo, hi);
        ...
        pr_info("IA32_VMX_EXIT_CTLS MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vmexit, 11, lo, hi);
